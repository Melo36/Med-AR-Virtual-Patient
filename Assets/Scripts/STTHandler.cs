...
public class STTHandler : MonoBehaviour
{
    ...
    public bool playerStartedSpeaking = false;
    ...
    void Start()
    {
        ...
        {
            microphoneDevice = Microphone.devices[0]; // Use the first available device
        ...
        else
        {
            ...
            enabled = false; // Disable script if no microphone
            return;
        }
    ...
    private IEnumerator PerformServerCheckCoroutine(float timeoutSeconds = 3.0f)
    {
        ...
        else
        {
            ...
            Debug.Log($"Server check completed. Server is {(result ? "reachable" : "NOT reachable")}.");
        }
    ...
    private void HandleFinalTranscription(string transcribedText)
    {
        ...
        if (llmhandler != null)
        {
            if (!string.IsNullOrEmpty(transcribedText)) // Optional: Check if text is not empty
            {
                ...
                llmhandler.Question("Doctor :" + transcribedText);
        ...
        try
        {
            ...
            switch (dataMessage.type)
            {
                ...
                case "recording_start":
                    playerStartedSpeaking = true;
                    Debug.Log("Server indicated recording started.");
                    OnRecordingStarted?.Invoke();
                    break;
                case "recording_stop":
                    playerStartedSpeaking = false;
