...
public class KokoroTTSSpeaker : MonoBehaviour, ILocalTTSSpeaker
{
    ...
    public bool personSpeaking = false;
    ...
    public void Speak(string textToSynthesize)
    {
    ...
    private void EnqueueAudio(byte[] audioData)
    {
        ...
        if ((!isPlaying && !delayPlayback) || (!isPlaying && llmHandler.isSelf))
        {
    ...
    {
        if (!isPlaying && delayPlayback && !llmHandler.isSelf)
            StartCoroutine(PlayQueuedAudio());
    }
